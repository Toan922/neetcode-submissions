...
public:
    ...
    string get(string key, int timestamp) {
        ...
        //init the result vector that will keep track of all previously called set values prior to timestamp
        //init w/ an empty string such that if the whole loop completes and the "" is the most recent -> no such val
        vector<string> valueBeforeTimestamp = {""};
        ...
        while(left <= right)
        {
            ...
            if(keyValueTime[middle].second <= timestamp)
            {
                ...
                valueBeforeTimestamp.push_back(keyValueTime[middle].first);
        ...
        return valueBeforeTimestamp.back();
    }
};
